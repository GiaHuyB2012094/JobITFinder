...
const Editor = ({ sections, information, setInformation }) => {
  ...
  const [values, setValues] = useState({
    ...
    avatarImg: activeInfo?.detail?.avatarImg || "",
  });
      ...
      resultInputGroup = (
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <label className="text-gray-600 text-sm mb-1 font-medium">
                Mô tả chi tiết các chức năng & nhiệm vụ của dự án
              </label>
              <button
                className="flex items-center py-1 px-2 bg-transparent border-solid border-[#239ce2] border-2 rounded-md text-[#239ce2] outline-none font-medium text-base
                                        active:translate-y-1"
  ...
  useEffect(() => {
    ...
    setValues({
      ...
      avatarImg: activeInf?.detail?.avatarImg || "",
    });
  }, [actionSectionKey]);
  ...
  useEffect(() => {
    ...
    setValues((prev) => ({ ...prev, avatarImg: imageUrl[0] }));
    const tempDetail = {
      name: values.name,
      title: values.title,
      linkedin: values.linkedin,
  ...
  useEffect(() => {
    ...
    setValues({
      ...
      certificateLink:
        activeInf.details[activeDetailIdx]?.certificateLink || "",
  ...
  return (
    <section className=" w-2/5 min-h-[450px] h-fit flex flex-col gap-7 shadow-md pt-2 bg-slate-100 border">
            ...
            ? activeInfo?.details?.map((item, idx) => (
                ...
                >
                  <p className="text-base text-white">
                    {sections[actionSectionKey]} + {idx + 1}
                  </p>
                  <IoClose
