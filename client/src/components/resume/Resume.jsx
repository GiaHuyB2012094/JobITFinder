...
const Resume = forwardRef(
  ({ information, sections, sizeSection, setSizeSection }, ref) => {
    ...
    const sectionDiv = {
      [sections.workExp]: (
        ...
        >
          <div className="text-2xl font-medium w-full border-b-2 border-solid border-slate-400">
            {info.workExp?.sectionTitle}
          </div>
          <div className="ml-2 flex flex-col gap-3 pb-2 px-0">
            {info.workExp?.details?.map((item, idx) => (
              <div
                className={`${
                  idx > 0 ? "border-t-2 border-solid border-slate-200 pb-2" : ""
                }`}
                key={item.title + idx}
              >
                {item.title && (
          ...
          <div className="ml-2 flex flex-col gap-1 pb-2 px-0">
            {info.education?.details?.map((item, idx) => (
              <div key={idx} className="">
                ...
                {item.college && (
                  <p style={colorTextItem} className=" font-medium ">
                    {item.college}
                  </p>
                )}
                {item.startDate && item.endDate ? (
      ...
      [sections.summary]: (
        ...
        >
          ...
          <div className="ml-2 flex flex-col gap-3 pb-1 px-0">
            <p className="font-normal text-xl">{info.summary?.detail}</p>
          </div>
        </div>
      ),
      [sections.other]: (
        ...
        >
          ...
          <div className="ml-2 flex flex-col gap-3 pb-1 px-0">
            <p className="font-normal text-xl">{info.other?.detail}</p>
          </div>
        </div>
      ),
    };
    const swapSourceTarget = (source, target) => {
      ...
      tempCols[sourceColumnIndex][sourceRowIndex] =
        tempCols[targetColumnIndex][targetRowIndex];
    ...
    useLayoutEffect(() => {
      ...
      // container.style.cssText = `height: 0px`
    }, [colorActive]);
    ...
    useEffect(() => {
      ...
      inf.forEach((item) => {
        if (
          ...
          information[item]?.details?.length > 0 ||
          information[item]?.points?.length > 0
        ) {
          count += 1;
        }
      });
    ...
    useEffect(() => {
      ...
      if (sum > 1060) setSizeSection(sum);
      else setSizeSection(1060);
    }, [bodyRef.current?.offsetHeight]);
    return (
      <div className=" bg-gray-200 w-3/5 h-[750px] my-0 mx-auto relative ">
        <Box
          sx={{
            width: "98%",
            mt: "1rem",
        ...
        <section
          className="absolute left-0 right-0 flex-1 w-[820px] h-[1060px] bg-white gap-8 mt-0 mx-auto overflow-y-scroll no-scrollbar"
          ref={ref}
        >
          ...
          <div ref={bodyRef} className="w-full">
            <div className="flex flex-col gap-1 px-7">
              <p
                style={colorTextItem}
                className=" text-5xl font-medium capitalize m-0"
              >
              ...
              <div className="my-1 flex flex-wrap gap-2">
                {info.basicInfo?.detail?.email && (
                  <a className="flex items-center text-base" type="email">
                ...
                <div className="w-52 mt-2 h-52">
                  {info.basicInfo?.detail?.avatarImg ? (
                    <img
                      ...
                      src={info.basicInfo?.detail?.avatarImg}
                      alt="avatar"
                    />
                  ) : (
                    ""
