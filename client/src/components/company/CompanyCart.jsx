...
const CompanyCart = (props) => {
  const { title, image, quantity, address, skills, idCompany } = props;
  return (
    <>
      <Link to={`/company-item/${idCompany}/${title}`}>
        <div
          className="flex-center cursor-pointer min-w-[700px] h-40 gap-x-5 p-3 bg-white rounded-md shadow-md
                    hover:bg-indigo-50 hover:border-solid hover:border-2 hover:border-indigo-200"
        >
          <div className="w-1/5 flex-center h-[150px] p-5">
            <Image src={image} alt="SignInImage" className="w-full h-fit" />
          </div>
          <div className="w-4/5 h-full">
            <p className="text-lg font-medium text-indigo-500 line-clamp-1">
              {title}
            </p>
            <div className="">
