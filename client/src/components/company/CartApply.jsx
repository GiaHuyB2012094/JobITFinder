...
import { useConformApplyMutation, useRejectApplyMutation } from "../../slices/applyApiSlice";
...
const CartApply = ( 
  ...
  const [ conformApply ] = useConformApplyMutation({ refetchOnMountOrArgChange : true });
  const [ rejectApply ] = useRejectApplyMutation({ refetchOnMountOrArgChange : true });
  const [ openFormSchedule, setOpenFormSchedule ] = useState(false)
  const handleConfirm = async() => {
    try {
      await conformApply(idApply).unwrap();
      Swal.fire({
        title: "Xác nhận",
  ...
  const handleRejectApply = async() => {
    try {
      await rejectApply(idApply).unwrap();
      Swal.fire({
        title: "Từ chối",
  ...
  return (
    <>
      <div className="w-full p-5 bg-slate-50 rounded-md shadow-sm flex-center gap-2 border-2 border-solid border-indigo-100
        ...
        <div className="w-8/12 flex flex-col gap-3">
          <div className="flex gap-1">
            ...
            <div className="font-medium text-xs ">
              {status === 'await' 
                ? (<p className="bg-amber-300 text-white px-2 py-1 mx-2  rounded-md">Đang chờ</p>) 
                : status === 'successful'
                  ? <p className="bg-[#0BC1B6] text-white px-2 py-1 mx-2  rounded-md">Đã xác nhận</p>
                  :  status === "scheduled"
                    ? <p className="bg-indigo-500 text-white px-2 py-1 mx-2  rounded-md">Đã lên lịch hẹn</p>
                    : status === "cancel" 
                      ? <p className="bg-red-400 text-white px-2 py-1 mx-2  rounded-md">Đã hủy</p>
                      : ""}
            </div>
            <Popover 
              content={
                <p className="flex flex-col flex-wrap">mkasdkdakms</p>
              } 
              // title="Title"
            >
              <p className="px-2 py-1 rounded-md border-2 border-solid border-gray-400 hover:bg-black hover:text-white"><SlEnvolopeLetter/></p>
            </Popover>
          </div>
          <div className="">
            <p className="text-indigo-500 overflow-hidden font-medium">{post?.name}</p>
          </div>
  
          <div className="flex gap-5 ">
              <div className="flex-center gap-2">
                <p className="text-base font-medium text-indigo-600"><FaPhoneAlt/></p>
                <p className="text-base">{phone}</p>
              </div>
              <div className="flex-center gap-2">
                <p className="text-xl font-medium text-indigo-600"><MdEmail/></p>
                <p className="text-base">{email}</p>
              </div>
              <div className="flex-center gap-2">
                <p className="text-base font-medium text-indigo-600"><FaClock/></p>
        ...
        <div className="w-2/12 flex flex-col gap-4 mx-1">
          <div className="flex flex-col items-end mb-6">
            <p className="italic flex-center gap-2 text-indigo-500 px-2 py-1  border-b-2 border-solid border-indigo-300
              hover:bg-indigo-500 hover:text-white hover:border-indigo-500 cursor-pointer "
              onClick = {() => {window.open(`${URL_Server}/files/${cv.pdf}`,"_blank","noreferrer")}}
              >
               <span><FaRegEye/></span> Xem CV
            </p>
          </div>
      ...
      <Modal
        title={
          <p className="text-lg pb-3 border-b-2 border-solid border-orange-200">
            ...
            <span className="mx-1 font-bold text-orange-500">
              {name}
            </span>
            với công việc 
            <span className="mx-1 font-bold text-orange-500">
