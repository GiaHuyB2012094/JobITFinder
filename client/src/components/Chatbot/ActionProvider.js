export default class ActionProvider {
  ...
  greet() {
    const greetingMessage = this.createChatBotMessage(
      "Chào bạn, rất vui khi được trò chuyện cùng bạn"
    );
  ...
  handleShowListTopCompanies = () => {
    const message = this.createChatBotMessage(
      "Dưới đây là danh sách top công ty trên trang tuyển dụng",
      {
  ...
  handleThanks = () => {
    const message = this.createChatBotMessage(
      "Cảm ơn bạn, cần thêm thông tin hay trao đổi thì nhắn vào đây"
    );
    this.updateChatbotState(message);
  };
}
