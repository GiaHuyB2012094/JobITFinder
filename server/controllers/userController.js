...
const registerUser = asyncHandler(async (req, res) => {
    ...
    // user---------------------------
        ...
        const user = await User.create({lastName, firstName, email, password, position})
        if (user) {
            generateToken(res, user._id)
            res.status(201).json({
                _id: user._id,
...
const getAllCompany = asyncHandler(async (req, res) => {
    ...
    if (companyList) {
        return res.status(200).json(companyList.sort((a,b) => 0.5 - Math.random()));
    } else {
        res.status(400);
        throw new Error('Invalid user data');
    }
...
const updateUserProfile = asyncHandler(async (req, res) => {
    ...
    if (user) {
        if (user.position === "company") {
            ...
            user.industryCompany = req.body.industryCompany || user.industryCompany || [];
            user.skillOfCompany = req.body.skillOfCompany || user.skillOfCompany || [];
