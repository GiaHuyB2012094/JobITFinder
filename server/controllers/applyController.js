...
const createApply = asyncHandler(async (req, res) => {
    ...
    const { name, email, phone, cv, intro, companyID, post, userID, status } = req.body;
    ...
    postExit.apply += 1;
        const apply = await Apply.create({ name, email, phone, cv, intro, companyID, post, userID, status, interviewSchedule : {} });
    if (apply) { 
        postExit.save();
